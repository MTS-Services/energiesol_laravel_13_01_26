...
import { update, index } from '@/actions/App/Http/Controllers/Admin/FeatureController';
...
export default function EditFeature({ feature }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    _method: 'put',
    title: feature.title,
    description: feature.description,
    image: null as File | null,
    action_text: feature.action_text,
    action_url: feature.action_url,
  });

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    post(update.url(feature.id), {
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid gap-2">
              ...
              />
              {feature.image && (
                <p className="text-sm text-gray-500">Current image: <a href={`/storage/${feature.image}`} target="_blank" rel="noopener noreferrer">{feature.image.split('/').pop()}</a></p>
              )}
